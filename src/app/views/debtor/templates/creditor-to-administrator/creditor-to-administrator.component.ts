...
interface Administrator { name: string, number: string, address: string }

interface ContractCreditorToAdministrator extends Contract {
  place: string, number: string, creditorToAdministratorDate: Date, insolvencyDate: Date, insolvencyRegisterEntryDate: Date, basis: Basis, administrator: Administrator, attachments: any, saveDoc: boolean
}

@Component({
  selector: 'app-creditor-to-administrator',
  templateUrl: './creditor-to-administrator.component.html',
  styleUrls: ['./creditor-to-administrator.component.css']
})
export class CreditorToAdministratorComponent implements OnInit {
  ...
  onSubmit() {
    ...
    this.contract.place = this.creditorToAdministratorForm.controls['place'].value;
    this.contract.number = this.creditorToAdministratorForm.controls['number'].value;
    this.contract.creditorToAdministratorDate = this.creditorToAdministratorForm.controls['creditorToAdministratorDate'].value;
    this.contract.basis = this.creditorToAdministratorForm.controls['basis'].value;
    this.contract.insolvencyDate = this.creditorToAdministratorForm.controls['insolvencyDate'].value;
    this.contract.insolvencyRegisterEntryDate = this.creditorToAdministratorForm.controls['insolvencyRegisterEntryDate'].value;

    this.contract.administrator.name = this.creditorToAdministratorForm.controls['administratorName'].value;
    this.contract.administrator.number = this.creditorToAdministratorForm.controls['administratorNumber'].value;
    this.contract.administrator.address = this.creditorToAdministratorForm.controls['administratorAddress'].value;

    this.contract.attachments = this.creditorToAdministratorForm.controls['attachments'].value
    this.contract.saveDoc = this.creditorToAdministratorForm.controls['saveDoc'].value;

    this.http.post<any>(`${environment.apiUrl}/pdf/contract/creditor/to/administrator`,
      {
